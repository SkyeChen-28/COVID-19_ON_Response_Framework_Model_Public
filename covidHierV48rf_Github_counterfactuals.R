#     SIMULATION
#     Script runs simulations for counterfactuals for 10 March 2020 to 7 February 2021 (varying response framework thresholds and time frames)
...
output_dir = "OutputFiles/FCF" # Directory to store output files
...
# # Varying thresholds 
# mu_vec = c(.1, .5, 1, 2) # Controls the strictness of the thresholds for each indicator. (Default value = 1)
# rf_start_vec = c(241,241,241,241); #Day that the response framework system got implemented (Default value = 241 representing Nov 7, 2020).
# rf_end_vec = c(290,290,290,290); # Day that we're suspending the response framework (Default value = 290 due to provincial shutdown on Dec 26th).
# my_length_vec <- c(425,425,425,425); # Number of days to run the simulation for (Default value = 375)

# Varying time frame
mu_vec = c(1,1,1,1,1) # Controls the strictness of the thresholds for each indicator. (Default value = 1)
rf_start_vec = c(241,211,79+14,241,241); #Day that the response framework system got implemented (Default value = 241 representing Nov 7, 2020).
rf_end_vec = c(290,290,290,320,400); # Day that we're suspending the response framework (Default value = 290 due to provincial shutdown on Dec 26th).
my_length_vec <- c(425,425,425,425,425); # Number of days to run the simulation for (Default value = 375)

# # Default only
# mu_vec = c(1) # Controls the strictness of the thresholds for each indicator. (Default value = 1)
# rf_start_vec = c(241); #Day that the response framework system got implemented (Default value = 241 representing Nov 7, 2020). 
# rf_end_vec = c(290); # Day that we're suspending the response framework (Default value = 290 due to provincial shutdown on Dec 26th).
# my_length_vec <- c(375); # Number of days to run the simulation for (Default value = 375)
###^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Control panel ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^###

N_scene = length(mu_vec)
for (n in 1:N_scene){ # For running N_scene different counterfactual scenarios

# For loading variables representing the different scenarios
mu = mu_vec[n]
rf_start = rf_start_vec[n]
rf_end = rf_end_vec[n]
my_length = my_length_vec[n] 
  
# Display which scenario we're running
# message("mu = ", mu, "\n")
message("rf_start = ", rf_start, "\n")
message("rf_end = ", rf_end, "\n")

# Name the experiment
# exp_name = sprintf("_thresholds=%s", mu) # For varying indicators
exp_name = sprintf("_rf(start,end)=(%s,%s)", rf_start, rf_end) # For varying response framework start and end dates
print(exp_name)

# ### For differing initial shutdown periods (DEPRECATED, do not remove, parts of the code require this)
init_shutdown <- init_s_vec[1] # Initial shutdown length
# cat("Initial Shutdown Length:", init_shutdown, "\n")
# exp_name<-paste(init_shutdown, "_shutdown", "_", my_length, "days", sep = ""); 
...
topnum <- 10
fitparms00 = readRDS(sprintf("%s/simsFITPARMS_%s.rds", input_dir, codeversion));
...
parms_len = dim(fitparms)[2] - 2

goodfits<-1; # Do NOT modify this number, it should always be 1 at this point in the code!
for (fit in 1:topnum) {
  
  parms=as.matrix(fitparms[fitparms$fit==unique(fitparms$fit)[fit],1:parms_len]); # use best fit from fitting
...
for (run in 1:nrun) {
  ...
  df_region$mu <- mu
  df_region$rf_start <- rf_start
  df_region$rf_end <- rf_end
  ...
  # df.length = length(df)
  # df.reduced<-df[,c(56:67,117:df.length)] # Take out the travel matrix and the parameters? Find a better, dynamic way to do this!
  # df_region.reduced<-df_region[,56:100]
  
  # Reduce the dataframes by taking out the state space (SEPAIR) parameters, and the travel matrix for df. 
...
df.fitparms$mu <- mu
df.fitparms$rf_start <- rf_start
df.fitparms$rf_end <- rf_end
...
message("(scenario, fit) = ", sprintf("(%s, %s)", n, fit_num), " finished")

goodfits<-goodfits+1;

} 

saveRDS(df.region.comb, file = sprintf("%s/simsREGIONS_%s%s_V2.rds", output_dir, codeversion, exp_name));
saveRDS(df.prov.comb, file = sprintf("%s/simsPROVINCE_%s%s_V2.rds", output_dir, codeversion, exp_name));
saveRDS(df.fitparms.comb, file = sprintf("%s/simsFITPARMS_%s%s_V2.rds", output_dir, codeversion, exp_name));
cat("Saved to", sprintf("%s/simsREGIONS_%s%s_V2.rds", output_dir, codeversion, exp_name), "\n")
} # This is the closing brace that controls the loop I made for running N_scene different counterfactual scenarios
