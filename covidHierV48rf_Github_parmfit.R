#     PARMFIT
#     Script runs fitting algorithms for COVID-19-ON-model
...
output_dir = "OutputFiles/Parmfit" # Directory to store output files
...
# These variables help ensure that the simulation is in fitting mode, they should all be FALSE, don't modify them.
...
# Commented out the nudge for testing, add it back in when done!
parmStr=parmStr0*(1.1-0.2*runif(length(pfit))); #Add a little "nudge" to start values of no greater than 20% off our "best guess" of parameter set (based on previous rounds of fitting)
while (any(parmStr < lowerBex | parmStr > upperBex)) {
  parmStr=parmStr0*(1.1-0.2*runif(length(pfit))); 
  # print("Reset IC"); 
  }
lowerBex=lowerBex[pfit]; upperBex=upperBex[pfit];

#Fitting algorithms. Note that the primary algorithm used (ISRES) is stochastic, so results may differ between runs.(4435, 887)
x=nloptr(parmsFit=fitnames[pfit],opts=list("algorithm"="NLOPT_GN_ISRES","xtol_rel"=1e-5,"maxeval"=10,"print_level"=2), x0=parmStr[pfit],eval_f=LLfun,lb=lowerBex[pfit],ub=upperBex[pfit],reps=nrep,extras=extras);
y=nloptr(parmsFit=fitnames[pfit],opts=list("algorithm"="NLOPT_LN_COBYLA","xtol_rel"=1e-15,"maxeval"=2,"print_level"=2), x0=x$solution,eval_f=LLfun,lb=lowerBex[pfit],ub=upperBex[pfit],reps=nrep,extras=extras);

#Get time to create unique identifier for output from fitting algorithm run
clock<-unclass(as.POSIXlt(Sys.time()))

# Save output from fitting algorithm run
saveRDS(x, file = sprintf("%s/parmfit_x_%s_d%ih%im%ims%d.rds", output_dir, codeversion, clock$mday, clock$hour, clock$min, round(1000*clock$sec)));
saveRDS(y, file = sprintf("%s/parmfit_y_%s_d%ih%im%ims%d.rds", output_dir, codeversion, clock$mday, clock$hour, clock$min, round(1000*clock$sec)));
